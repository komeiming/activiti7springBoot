  ...
  <div class="login-container">
    <div class="login-box">
        ...
        <el-form :model="loginForm" :rules="rules" ref="loginFormRef" :validate-on-rule-change="false">
          ...
          <el-form-item>
            <div style="display: flex; justify-content: space-between; align-items: center;">
              ...
              <el-button type="text" size="small" @click="goToForgotPassword">忘记密钥？</el-button>
            </div>
          </el-form-item>
      ...
      <div class="login-footer">
        ...
        <p>系统用户？<el-button type="text" @click="goToSysLogin">前往系统用户登录</el-button></p>
...
import { Key as KeyIcon, Lock as LockIcon, CircleCheck } from '@element-plus/icons-vue'
...
const rules = {
  appId: [
    { required: true, message: '请输入APP ID', trigger: 'submit' }
  ],
  secretKey: [
    { required: true, message: '请输入密钥', trigger: 'submit' }
  ],
  captcha: [
...
const handleLogin = async () => {
  ...
  try {
    // 调用租户登录API
    const response = await axios.post('/auth/tenant/login', {
    ...
    // 创建租户用户信息对象
    const userInfo = {
      username: loginForm.appId,
      token: response.token || 'token-' + Date.now(),
      name: response.name || loginForm.appId,
      role: response.role || 'tenant',
      roles: response.roles || ['ROLE_USER', 'tenant'],
      appId: loginForm.appId,
      loginTime: new Date().toISOString(),
      userType: 'tenant' // 标记为租户用户
    }
    ...
    localStorage.setItem('tenantInfo', JSON.stringify({
      appId: loginForm.appId,
      secretKey: loginForm.secretKey,
      serviceModules: response.serviceModules || '',
      name: response.name || loginForm.appId
    }))
    
    // 如果选择记住我，保存APP ID
    if (loginForm.rememberMe) {
      localStorage.setItem('rememberedTenantAppId', loginForm.appId)
    } else {
      localStorage.removeItem('rememberedTenantAppId')
    }
    
    console.log('租户用户登录成功:', userInfo)
    ...
    // 提供更具体的错误信息
    errorMessage.value = error.responseData?.message || error.response?.data?.message || error.message || '登录失败，请检查APP ID和密钥'
...
// 组件挂载时检查是否有记住的租户APP ID
onMounted(() => {
  // 异步加载本地存储数据，减少初始化阻塞
  setTimeout(() => {
    const savedAppId = localStorage.getItem('rememberedTenantAppId')
    if (savedAppId) {
      loginForm.appId = savedAppId
      loginForm.rememberMe = true
    }
    
    // 检查是否需要显示验证码（如果之前有连续登录失败记录）
    const savedFailCount = localStorage.getItem('tenantLoginFailCount')
    if (savedFailCount) {
