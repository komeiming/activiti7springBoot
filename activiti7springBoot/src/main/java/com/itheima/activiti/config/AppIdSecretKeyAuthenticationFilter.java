...
public class AppIdSecretKeyAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) 
        ...
        // 检查是否为多部分请求，多部分请求不能使用BodyCachingHttpServletRequestWrapper
        ...
        if (!isMultipartRequest) {
            // 包装请求，以便后续可以重复读取body
            requestToUse = new BodyCachingHttpServletRequestWrapper(request);
        }
        ...
        if (((requestUri.startsWith("/api/notification") || requestUri.startsWith("/api/workflow")) || 
            ...
            String authorization = requestToUse.getHeader("Authorization");
            ...
            String appId = requestToUse.getHeader("X-APP-ID");
            String signature = requestToUse.getHeader("X-SIGN");
            ...
            // 获取请求方法
            String method = requestToUse.getMethod().toUpperCase();
            ...
            if (("POST".equals(method) || "PUT".equals(method)) && !isMultipartRequest) {
                try {
                    // 读取请求body
                    String body = ((BodyCachingHttpServletRequestWrapper) requestToUse).getBody();
